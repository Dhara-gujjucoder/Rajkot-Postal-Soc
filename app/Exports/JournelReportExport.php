...
class JournelReportExport implements FromCollection, WithMapping, ShouldAutoSize, WithStrictNullComparison, WithStyles, WithDrawings
{
    protected $month, $data;
    ...
    public function map($bulk_entry_master): array
    {
        ...
        $entry = [
            ...
            ['', '', '', getSetting()->title],
            ['', '', '', getSetting()->address],
            ['', '', '', ($bulk_entry_master->department->department_name)],
            [],
            [],
        ...
        foreach ($bulk_entry_master->bulk_entries as $key => $value) {
            ...
            $entry[] = [
                '', $key+1, $value->member->uid, $value->member->name ?? '', $bulk_entry_master->rec_no, $value->principal, $value->interest, $value->fixed, $value->ms,  $value->principal+$value->interest+$value->fixed+$value->ms,
            ];
        }
        $entry[] = [];
